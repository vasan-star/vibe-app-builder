...
// Stop waiting long before Vercel's 5-minute function limit.
...
async function refundCredit(
  ...
  try {
    ...
    console.log("User credit refunded:", userId);
  } catch (error) {
    console.error(
...
function extractGeminiText(data) {
  ...
  // Compatibility with older Interactions API responses.
  if (
    data &&
...
async function callGemini(
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        "Gemini took too long to respond. Your credit has been refunded. Please try again in a moment."
      );
    }
...
export default async function handler(
  ...
  try {
    ...
    // happened after a credit was consumed.
    if (
      creditConsumed &&
      userId
    ) {
